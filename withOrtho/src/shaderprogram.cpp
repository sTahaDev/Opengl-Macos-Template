...
int ShaderProgram::addUniform(const std::string& varName)
{
    return m_UniformVars[varName] = glGetUniformLocation(m_ProgramId,varName.c_str());
}
...
int ShaderProgram::getUniform(const std::string& varName){
    return m_UniformVars[varName];
}
